...
public class MainViewController {
  ...
  private final ObjectProperty<SupplyerDTO> selectedSupplyer = new SimpleObjectProperty<>();
    ...
    public void initialize() throws IOException {
        var suppliers = FXCollections.observableArrayList(
                loadFXML("File"),
                loadFXML("Autex"),
                loadFXML("Complex")
        );

        selectedSupplyer.addListener((observableValue, oldSupplier, newSupplier) -> rootPane.setCenter(newSupplier.view));
  ...
  private Button getButton(SupplyerDTO supplyer) {
    var button = new Button(supplyer.name);
    var img = ofNullable(supplyer.imgPath)
    ...
    private SupplyerDTO loadFXML(String name) throws IOException {
        ...
        return new SupplyerDTO(name, fxmlLoader.load(), fxmlLoader.getController(), "/img/" + name + "_icon.png");
    }
